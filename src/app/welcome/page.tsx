...
const options = [
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        ...
        <path d="M7 8h8M7 12h5" stroke={W} strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        <path d="M11 2L13.5 8.5H20L14.5 12.5L16.5 19L11 15L5.5 19L7.5 12.5L2 8.5H8.5L11 2Z" stroke={M} strokeWidth="1.5" strokeLinejoin="round" />
      </svg>
    ),
  ...
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
        ...
        <path d="M9 8l5 3-5 3V8z" stroke={M} strokeWidth="1.5" strokeLinejoin="round" />
      </svg>
    ),
...
export default function WelcomePage() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: I, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 6, height: 6, borderRadius: "50%", background: WA(0.3) }} />
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: I, fontFamily: "var(--font-sans)", color: B }}>
      ...
      <nav style={{
        ...
        <Link href="/dashboard" style={{ fontSize: 13, color: M, textDecoration: "none" }}>
          Go to dashboard →
        </Link>
      </nav>

      {/* Content */}
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "72px 40px 40px" }}>

        {/* Greeting */}
        <div style={{ marginBottom: 56 }}>
          <div style={{
            display: "inline-flex", alignItems: "center", gap: 8,
            border: `0.5px solid ${WA(0.2)}`, padding: "5px 14px",
          ...
          <h1 style={{
            ...serif, fontSize: "clamp(32px,4vw,48px)", color: B,
            fontWeight: 400, lineHeight: 1.1, letterSpacing: "-1.2px", marginBottom: 16,
          }}>
            {displayName ? `Welcome, ${displayName}.` : "Welcome to ByteForm."}
          </h1>
          <p style={{ fontSize: 16, color: M, lineHeight: 1.75, maxWidth: 480 }}>
            You're all set. Pick where you'd like to start — your first form is just a couple of minutes away.
          </p>
        </div>

        {/* Option cards */}
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 56 }}>
          {options.map(({ icon, title, desc, cta, href, primary, disabled }) => (
            <div
              key={title}
              style={{
                background: "white", borderRadius: 14, padding: "24px 28px",
                border: primary ? `1.5px solid ${W}` : `0.5px solid ${WA(0.1)}`,
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start", gap: 18 }}>
                ...
                <div>
                  <p style={{ fontSize: 15, color: B, fontWeight: 500, marginBottom: 4 }}>{title}</p>
                  <p style={{ fontSize: 13, color: M, lineHeight: 1.6 }}>{desc}</p>
                </div>
              </div>
              {disabled ? (
                <span style={{
        ...
        <div style={{
          background: B, borderRadius: 14, padding: "28px 32px",
          ...
          {[
            ...
            <div key={label} style={{ textAlign: "center" }}>
              <div style={{ ...serif, fontSize: 24, color: I, fontWeight: 400, marginBottom: 4 }}>{n}</div>
              <div style={{ fontSize: 12, color: `rgba(247,243,236,0.4)` }}>{label}</div>
            </div>
          ))}
        </div>

        <p style={{ textAlign: "center", fontSize: 12, color: D, marginTop: 28 }}>
          Need help?{" "}
          <a href="mailto:team@skyl4b.com" style={{ color: W, textDecoration: "none" }}>
            team@skyl4b.com
          </a>
        </p>
      </div>
